## 概述  
分区计数窗口是窗口数据到达设定的个数后触发窗口计算，分区计数窗口的并行度可以设置。分区计数窗口编码流程为：inputStream.keyBy().countWindow().apply()    
...
```  
在DataStream中定义了countWindow方法，返回WindowedStream对象。其中WindowAssigner是通过GlobalWindows.create()生成的GlobalWindows对象
...
```  
WindowedStream构造函数的源码如下：  
```java
public class WindowedStream<T, W extends Window> {
	public WindowedStream(KeyedStream<T, K> input,
			WindowAssigner<? super T, W> windowAssigner) {
		this.input = input;
		this.windowAssigner = windowAssigner; //获取默认的触发器。
		this.trigger = windowAssigner.getDefaultTrigger(input.getExecutionEnvironment());
	}
}
```  
### WindowedStream.apply函数  
WindowedStream实现了apply方法，apply方法会构造一个WindowOperator对象，WindowOperator对象是OneInputStreamOperator的子类，是窗口核心实现。  
```java
